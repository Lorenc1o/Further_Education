...
double FEM<dim>::basis_function(unsigned int node, double xi_1, double xi_2){
  ...
  //EDIT
  if(node == 0){
    value = N1(xi_1)*N1(xi_2);
  } else if(node == 1){
    value = N2(xi_1)*N1(xi_2);
  } else if(node == 2){
    value = N1(xi_1)*N2(xi_2);
  } else if(node == 3){
    value = N2(xi_1)*N2(xi_2);
  }
...
std::vector<double> FEM<dim>::basis_gradient(unsigned int node, double xi_1, double xi_2){
  ...
  //EDIT
  if(node == 0){
    values[0] = N1prime(xi_1)*N1(xi_2);
    values[1] = N1prime(xi_2)*N1(xi_1);
  } else if(node == 1){
    values[0] = N2prime(xi_1)*N1(xi_2);
    values[1] = N1prime(xi_2)*N2(xi_1);
  } else if(node == 2){
    values[0] = N1prime(xi_1)*N2(xi_2);
    values[1] = N2prime(xi_2)*N1(xi_1);
  } else if(node == 3){
    values[0] = N2prime(xi_1)*N2(xi_2);
    values[1] = N2prime(xi_2)*N2(xi_1);
  }
...
void FEM<dim>::define_boundary_conds(){
  ...
  for(unsigned int i=0; i<totalNodes; i++){
    if(nodeLocation[i][1]==0){
      double x = nodeLocation[i][0];
      boundary_values[i] = 300*(1+c0*x);
    } else if(nodeLocation[i][1]==0.08){
      double x = nodeLocation[i][0];
      boundary_values[i] = 310*(1+c00*x*x);
    }
  }
}
              ...
              for(unsigned int j=0; j<dim; j++){
                for(unsigned int I=0;I<dim;I++){
                  for(unsigned int J=0;J<dim;J++){
                    ...
                    Klocal[A][B] += basis_gradient(A,quad_points[q1],quad_points[q2])[i]*invJacob[i][I]*kappa[I][J]*basis_gradient(B,quad_points[q1],quad_points[q2])[j]*detJ*quad_weight[q1]*quad_weight[q2];
                  }
                }
              }
            } 
