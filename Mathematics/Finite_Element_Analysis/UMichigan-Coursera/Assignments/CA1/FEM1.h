...
double FEM<dim>::basis_function(unsigned int node, double xi){
  ...
  int n_nodes_el = basisFunctionOrder + 1;

  for(int i=0; i<n_nodes_el; i++){
    if(i != node){
...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  for(int i=0; i<n_nodes_el; i++){
    if(i!=node){
      double aux = 1/(xi_at_node(node)-xi_at_node(i));
      for(int k=0; k<n_nodes_el; k++){
        if (k!=i && k!=node){
          aux *= (xi - xi_at_node(k));
          aux /= (xi_at_node(node)-xi_at_node(k));
        }
      }
      value += aux;
    } 
...
/*quadRule = 4; //EDIT - Number of quadrature points along one dimension
  ...
  quad_points[2] = -sqrt(3./7. - 2./7. * sqrt(6./5.)); //EDIT
  quad_points[3] = sqrt(3./7. - 2./7. * sqrt(6./5.));
  quad_points[1] = sqrt(3./7. + 2./7. * sqrt(6./5.)); //EDIT


  quad_weight[0] = (18-sqrt(30))/36.; //EDIT
  quad_weight[1] = (18-sqrt(30))/36.; //EDIT
...
double u_exact_p1(double x){
  ...
  double c1 = (g2-g1)/L+f/(6*E)*L*L;
  double c2 = g1;
  double a = -f/(6*E);

  return a*x*x*x+c1*x+c2;
}


double u_exact_p2(double x){
  double g1 = 0.;
  double g2 = 0.001;
  double L = 0.1;
  double f = 1e11;
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int q=0; q<quadRule; q++){
      ...
      if(prob == 1){
        u_exact = u_exact_p1(x);
      } else if(prob == 2){
        u_exact = u_exact_p2(x);
      }
      double e = u_exact - u_h;
      e = e*e;
      l2_norm_aux += e*h_e/2*quad_weight[q];
    }
